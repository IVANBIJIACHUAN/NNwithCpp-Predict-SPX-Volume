...
public:
	...
	Eigen::MatrixXd BackwardPropagation(const Eigen::MatrixXd& gradient, Optimizer o);
...
private:
	...
	Eigen::MatrixXd direction_weight;
	Eigen::MatrixXd direction_b;
};
...
DenseLayer::DenseLayer(int _backunits_len, int _units_len, string name, double _learning_rate = 0.03, bool _is_input_layer = false, Activation t = DenseLayer::Sigmoid) :
	...
	gradient_weight(_backunits_len, _units_len), gradient_b(1, _units_len), layer_name(name), direction_weight(_backunits_len, _units_len), direction_b(1, _units_len)
{
	...
	act_func = t;
	gradient_weight = Eigen::MatrixXd::Zero(backunits_len, units_len);
	gradient_b = Eigen::MatrixXd::Zero(1, units_len);
	direction_weight = Eigen::MatrixXd::Zero(backunits_len, units_len);
	direction_b = Eigen::MatrixXd::Zero(1, units_len);
...
Eigen::MatrixXd DenseLayer::BackwardPropagation(const Eigen::MatrixXd &gradient, Optimizer o = Optimizer::GD)
{
	//partial loss/ partial wij= 1{wx_plus_b[i]>=0} * xdatai * gradientj
	Eigen::MatrixXd gradient_activation = cal_gradient();
	Eigen::MatrixXd gradient_weight_last = gradient_weight;
	Eigen::MatrixXd gradient_b_last = gradient_b;
	...
	if (o == Optimizer::CGD)
	{
		double beta1_weight = gradient_weight.squaredNorm();
		double beta2_weight = gradient_weight_last.squaredNorm();
		double beta1_b = gradient_b.squaredNorm();
		double beta2_b = gradient_b_last.squaredNorm();
		if (beta2_weight != 0)
		{
			double beta_weight = beta1_weight / beta2_weight;
			if (beta_weight > 1)
				beta_weight = 1;
			direction_weight = -gradient_weight - beta_weight * direction_weight;
			weight = weight + learning_rate * direction_weight;
		...
		if (beta2_b != 0)
		{
			double beta_b = beta1_b / beta2_b;
			if (beta_b > 1)
				beta_b = 1;
			direction_b = -gradient_b - beta_b * direction_b;
			bias = bias + learning_rate * direction_b;
...
public:
	...
	void BuildLayer();
	void Summary();
	double Train(const Eigen::MatrixXd& xdata, const Eigen::MatrixXd& ydata, int _train_round, double _accuracy, int validnum, DenseLayer::Optimizer o);
...
double BPNN::Train(const Eigen::MatrixXd& xdata, const Eigen::MatrixXd& ydata, int _train_round, double _accuracy, int validnum = 3, DenseLayer::Optimizer o = DenseLayer::GD)
{
	...
	for (int i = 0; i < train_round; i++)
	{
		...
		for (int j = 0; j < xdatatrain.rows(); j++)
		{
			...
			{
				loss_gradient = layers[layers.size() - 1 - k]->BackwardPropagation(loss_gradient,  o);
			}
		}
...
void real_train()
{
	...
	DenseLayer::Optimizer optimethod = DenseLayer::CGD;
	BPNN modelnew;
	...
	modelnew.Train(xdata_reduce, ydata, 250, 0.25, 30, optimethod); // xdata, ydata, train_round
